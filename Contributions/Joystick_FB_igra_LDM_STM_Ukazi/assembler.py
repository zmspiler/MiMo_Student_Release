...
with open(sys.argv[1]) as f:
    for ORline in f.readlines():

        ORline = re.sub("\s*#.*", "", ORline)
        ORline = re.sub("\t\t", "\t", ORline)
        ORline = ORline.strip()
        ORline_ = ORline
        if re.match("^\s*$", ORline):
            continue

        # Deal with labels
        tmp = re.match("(.*):\s+(.*)", ORline)
        if tmp:
            Label[tmp.group(1)] = PC
            ORline = tmp.group(2)

        op = re.split("\s+", ORline, 1)
        lines = []
        if op[0] == "stm":
            first = True
            temp = re.split(".*{", ORline)[1][:-1].split("-")
            lower = int(temp[0][1])
            if(len(temp) == 1):
                upper = lower
            else:
                upper = int(temp[1][1])
            if (lower < 0 or upper > 7 or upper < lower):
                sys.exit("Bad args: " + ORline + "\n")
            for i in range(lower, upper+1):
                if first and tmp:
                    first = False
                    OriginalLine.append(f"{tmp.group(1)}   push r{i}")
                    lines.append(f"push r{i}")
                else:
                    OriginalLine.append(f"push r{i}")
                    lines.append(f"push r{i}")
        elif op[0] == "ldm":
            first = True
            temp = re.split(".*{", ORline)[1][:-1].split("-")
            ...
            for i in range(upper, lower-1, -1):
                if first and tmp:
                    first = False
                    OriginalLine.append(f"{tmp.group(1)}   pop r{i}")
                    lines.append(f"pop r{i}")
                else:
                    OriginalLine.append(f"pop r{i}")
                    lines.append(f"pop r{i}")
        else:
            OriginalLine.append(ORline_)
            lines.append(ORline)
...
for i in range(0, len(MCode)):
    print("{0}: {1} {2} {3}".format(hex(i)[2:].zfill(4), hex(MCode[i])[
          2:].zfill(8), "{0:016b}".format(MCode[i]), OriginalLine[i]))
