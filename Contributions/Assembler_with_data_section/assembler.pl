...
while (<$IN>) {
  ...
  die("Bad # args: $_. Expected ".length($Opcode{$opcode}->[1])." got ".@alist."\n") if (length($Opcode{$opcode}->[1]) != @alist && !$Data);
  ...
  foreach my $atype (split(//, $Opcode{$opcode}->[1])) {
    ...
    if ($atype eq 'w') {
      ...
      foreach my $ar (@args) {
	    ...
	    $PC--; # TODO: One too much increment in the LOOP 
	  }
...
for (my $i=0; $i < @Mcode; $i++) {
  ...
  if (!($label=~ m{^-?\d+$})) {
    die("Undefined label >$label< on line $i\n") if (!defined($Label{$label}));
    $label= $Label{$label};
  }
